$(function() {
	...
	var setTheme = function(style, color) {
		$.cookie('style', style);
		$.cookie('color', color);
		$('#theme-change').attr('href',
				'css/themes/' + style + '/' + color + '.css');
	}
	...
	$('.btn-fullscreen')
			.click(
					function() {
						...
						if (!document.fullscreenElement && // alternative
															// standard method
						!document.mozFullScreenElement
								&& !document.webkitFullscreenElement
								&& !document.msFullscreenElement) { // current
																	// working
																	// methods
	...
	$('#chat-form .chat-group a')
			...
			.click(
					function() {
						...
						var userStatus = $(this).find('span.user-status').attr(
								'class');
						$('#chat-box > .chat-box-header > span.user-status')
								.removeClass().addClass(userStatus);
						...
						var h_title = $("#chat-box > .chat-box-header")
								.height();
						var top = ($('#chat-box').is(':visible') ? (offset.top
								- h_title - 40) : (offset.top + h_title - 20));
					...
					function(e) {
						...
						if (e.which == 13) {
							...
							if ($content !== "") {
								...
								height += '';
								// alert(height);
								$me.scrollTop(height); // add more 400px for
														// #chat-box position
								...
								$res += "<span class='time'>" + h + ":" + m
										+ "</span>";
								$res += "</p>";
								$res = $res + "<p>" + "Yep! It's so funny :)"
										+ "</p>";
								$res += "</li>";
								setTimeout(function() {
									$me.append($res);
									$me.scrollTop(height + 100); // add more
																	// 500px for
																	// #chat-box
																	// position
								}, 1000);
	...
	$(".portlet").each(
			function(index, element) {
				var me = $(this);
				$(">.portlet-header>.tools>i", me).click(
						function(e) {
	...
	/*
	 * $('#news-update').ticker({ controls: false, titleText: '' });
	 */
	...
	$('.option-demo')
			.hover(
					function() {
						$(this)
								.append(
										"<div class='demo-layout animated fadeInUp'><i class='fa fa-magic mrs'></i>Demo</div>");
					}, function() {
						$('.demo-layout').remove();
					});
	$('#header-topbar-page .demo-layout')
			.live(
					...
					function() {
						...
						$('#header-topbar-option-demo').find('.demo-layout')
								.remove();
						return false;
					});
	$('#title-breadcrumb-page .demo-layout')
			.live(
					...
					function() {
						...
						$('#title-breadcrumb-option-demo').find('.demo-layout')
								.remove();
						return false;
					});
